...
export default function ButtonIcon({ icon, alt, onClick, link, target, width = "auto", height = "auto", parentStyles, text }: Props) {
    return (
        <div className={`${styles["button-container"]} ${(parentStyles != undefined) ? `${parentStyles["button-container"]}` : ""}`} onClick={onClick}>
            <Link href={link || ""} passHref>
                <a className={`${styles["button-link"]} ${(parentStyles != undefined) ? `${parentStyles["button-link"]}` : ""}`} target={target || "_self"}>
